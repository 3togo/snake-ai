...
class SnakeGame:
    ...
    def reset(self):
        self.snake = [
            (self.board_size // 2 + i, self.board_size // 2) for i in range(1, -2, -1)
    ...
    def step(self, action):
        ...
        if (
            row,
            col,
        ) == self.food:  # If snake eats food, it won't pop the last cell. The food grid will be taken by snake later, no need to update board vacancy matrix.
            food_obtained = True
            ...
            food_obtained = False
            self.non_snake.add(         # type: ignore
                self.snake.pop()
    ...
    def draw_game_over_screen(self):
        game_over_text = self.font.render("GAME OVER", True, (255, 255, 255))               # type: ignore
        final_score_text = self.font.render(                                                # type: ignore
            f"SCORE: {self.score}", True, (255, 255, 255)
        )
        retry_button_text = "RETRY"

        self.screen.fill((0, 0, 0))                                                         # type: ignore
        self.screen.blit(                                                                   # type: ignore
            game_over_text,
            (
        ...
        self.screen.blit(                                                                   # type: ignore
            final_score_text,
            (
                self.display_width // 2 - final_score_text.get_width() // 2,
    ...
    def draw_countdown(self, number):
        ...
        self.screen.blit(
            countdown_text,
            (
                self.display_width // 2 - countdown_text.get_width() // 2,
    ...
    def render(self):
        ...
        # Draw food
        if (
            len(self.snake) < self.grid_size
        ):  # If the snake occupies the entire board, don't draw food.
            r, c = self.food                                                                # type: ignore
            pygame.draw.rect(
                self.screen,                                                                # type: ignore
                (255, 0, 0),
                (
    ...
    def draw_snake(self):
        ...
        pygame.draw.circle(
            self.screen,
            (255, 255, 255),
            (head_x + eye_offset, head_y + eye_offset),
            eye_size,
        )
        pygame.draw.circle(
            self.screen,
            (255, 255, 255),
            (head_x + self.cell_size - eye_offset, head_y + eye_offset),
            eye_size,
        )
