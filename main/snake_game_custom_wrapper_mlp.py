...
class SnakeEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if self.done:  # Snake bumps into wall or itself. Episode is over.
            ...
            reward = math.exp(
                (self.grid_size - self.reward_step_counter) / self.grid_size
            )  # (0, e)
            ...
            if np.linalg.norm(
                ...
                reward = (
                    1 / info["snake_size"]
                )  # No upper limit might enable the agent to master shorter scenario faster and more firmly.
            else:
    ...
    def get_action_mask(self):
        return np.array(
            [[self._check_action_validity(a) for a in range(self.action_space.n)]]
        )
    ...
    def _check_action_validity(self, action):
        ...
        if (row, col) == self.game.food:
            game_over = (
                (row, col)
                in snake_list  # The snake won't pop the last cell if it ate food.
                or row < 0
